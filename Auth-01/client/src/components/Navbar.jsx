...
const Navbar = () => {
  const { navigate, axios, userData, setUserData, setIsLoggedin } =
    useAppContext();
    ...
    <div className="w-full flex justify-between items-center p-4 sm:p-6 sm:px-24 absolute top-0">
      ...
      {userData ? (
        <div className="w-8 h-8 flex justify-center items-center rounded-full bg-black text-white relative group">
          ...
          <div className="absolute hidden group-hover:block top-0 right-0 z-10 text-black pt-10 rounded">
            <ul className="list-none m-0 p-2 bg-gray-100 text-sm">
              ...
              <li
                onClick={logout}
                className="py-1 px-2 hover:bg-gray-200 cursor-pointer pr-12"
              >
                Logout
              </li>
            </ul>
        ...
        <button
          ...
          className="bg-black text-white text-md py-1.5 px-5 hover:rounded-full hover:border border-blue-600 transition-all cursor-pointer"
        >
          {" "}
          Login{" "}
        </button>
